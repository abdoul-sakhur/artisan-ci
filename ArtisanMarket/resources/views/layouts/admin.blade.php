        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-indigo-800 to-indigo-900 text-white transform lg:translate-x-0 lg:static lg:inset-0 transition-transform duration-200 ease-in-out"
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('admin.categories.index') }}" 
                   class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('admin.categories.*') ? 'bg-indigo-700 text-white' : 'text-indigo-100 hover:bg-indigo-700' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('admin.orders.index') }}" 
                   class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('admin.orders.*') ? 'bg-indigo-700 text-white' : 'text-indigo-100 hover:bg-indigo-700' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" 
                                ...
                                <div class="max-h-64 overflow-y-auto">
                                    @php
                                        $pendingArtisans = \App\Models\Artisan::where('is_approved', false)->count();
                                    @endphp
                                    @if($pendingArtisans > 0)
                                        <a href="{{ route('admin.artisans.approval') }}" class="block px-4 py-3 hover:bg-gray-50">
                                            <p class="text-sm text-gray-800">{{ $pendingArtisans }} artisan(s) en attente de validation</p>
